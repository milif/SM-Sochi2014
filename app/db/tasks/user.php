...
while (true){
    $rs = DB::query("SELECT id FROM `user` WHERE ref_key IS NULL LIMIT 0,500;");
    if(!count($rs)) break;
    foreach($rs as $item){
...
function _achievementLogIndexItem($item){
    ...
    DB::query("UPDATE `user` u INNER JOIN (SELECT a.id, GROUP_CONCAT(b.key) achievement FROM `user` a LEFT JOIN `achievement_log` b ON b.user_id = a.id WHERE `data_achievement_$item` IS NULL AND b.key LIKE '$item.%' GROUP BY a.id) a ON u.id = a.id SET u.`data_achievement_$item` = a.achievement");
}
