...
// СБОРКА МОДУЛЕЙ
  ...
  function _build(tempDir, config, doneTask){
    ...
    grunt.file.expand(config.lib).forEach(function(file){
        ...
        if(module) lib._requires.splice(0,0,'@requires ' + module[1] + '\n');
        if(name){
            libs[name] = lib;
        }
    }); 
    ...
    grunt.util.async.forEach(config.main, function(main, done){
        ...
        grunt.util._.forEach(main.include, function(include){
            var name = /\..*?:/.test(include) ? include : main.name + '.' + include;
            if(/\*$/.test(name)) {
                name = name.replace('*','');
        ...
        while(stack.length > 0){     
            ...
            if(current.requires.length > 0){
                ...
                if(require == '*') {
                    ...
                    if(stack.indexOf(libs[require]) < 0) stack.push(libs[require]);
                } else {
        ...
        grunt.file.expand(files).forEach(function(file){
            if(file.indexOf('.html') > 0){
                ...
                putAssetsFromCSS(file, assetList);
        ...
        if(main.includeAsset){
            jsAddContent += ';window._assets = (window._assets || []).concat('+ JSON.stringify(assetList) +');';
            map.assets = assetList;
        }
        var preloadedTemplates = [];
        var contentTemplates = '';
        grunt.util._.forEach(main.includeTemplates, function(files){
            grunt.file.expand(config.partialsDir + '/' + files).forEach(function(file){
                var tplCnt = grunt.file.read(file);
                preloadedTemplates.push(path.basename(file));
                contentTemplates += '<script type="text/ng-template" id="' + path.basename(config.partialsDir) + '/' + path.basename(file)+'">' + tplCnt
                    .replace(/\r?\n/g, '')
                    .replace(/ {2,}/g,' ')
                    .replace(/<!--.*?-->/g,'')
                 + '</script>';
        ...
        utils.min(file + '.js', function(){
            grunt.file.write(file + '.js', grunt.file.read(file + '.js') + jsAddContent);
            grunt.file.write(file + '.min.js', grunt.file.read(file + '.min.js') + jsAddContent);
            done();
        });
  ...
  function putAssetsFromCSS(file, assets){
    ...
    if(urls) for(var i=0;i<urls.length;i++){
        ...
        var asset = urlRegex.exec(urls[i])[1];
